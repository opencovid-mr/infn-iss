...
for regione in regioni:
	...
	df_nc = pd.DataFrame.from_records(zip(*datasets_nc), columns=['data', 'positivi']).dropna()

	df_c = pd.DataFrame.from_records(zip(*datasets_c), columns=['data','positivi']).dropna()

	result = df_c.append(df_nc).drop_duplicates(subset=['data'])

	if (regione[0]=='italia'): df_matrix = result
	else:
		df_matrix[regione[0]] = df_matrix['data'].map(result.set_index('data')['positivi'])
